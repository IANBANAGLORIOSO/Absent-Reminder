...
function doShowAll() {
	if (CheckBrowser()) {
		var key = "";
		var list = "<tr><th>number_of_absences</th><th>Date</th><th>statistics</th></tr>\n";
		var i = 0;
		for (i = 0; i <= localStorage.length - 2; i++) {
			key = localStorage.key(i);
			list += "<tr><td>" + key + "</td>\n<td>"
					+ JSON.parse(localStorage.getItem(key))[0] + "</td><td>"+ JSON.parse(localStorage.getItem(key))[1] +"</td></tr>\n";
		}
		if (list == "<tr><th>number_of_absences</th><th>Date</th><th>statistics</th></tr>\n") {
